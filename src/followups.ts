  ...
  if (state.status !== "completed") {
    const operatorNote = state.status === "paused" && state.pauseKind === "decision_pending" && state.decisionMode === "human"
      ...
      : state.status === "paused" && state.pauseKind === "workspace_drift"
        ...
        : state.status === "paused" && state.pauseKind === "reclaim_exhausted"
          ? `Run ${state.id} paused because a stage kept being reclaimed from a stale lease. Inspect that stage's attempts and events for the crash or stall, fix it, then run /prompt-chain-resume ${state.id}.`
