...
if (!plan) {
  process.stderr.write("usage: trip-compile <plan.md> [--out file.json] [--working-directory dir] [--mode auto|serial|parallel] [--allow-unresolved]\n");
  process.exit(64);
}
...
while (args.length) {
  ...
  if (token === "--out") output = args.shift();
...
const result = await compilePlanFile(plan, { outputPath: output, workingDirectory, mode, allowUnresolved });
