...
The compiler groups writer slices into dependency waves. A wave whose slices all declare parallel safety with non-overlapping, low-risk claims, and whose predecessor wave produced a checkpoint (or is wave 1), compiles to worktree writers plus a deterministic `checkpoint-wave-N` stage; other waves stay same-checkout and record why in `schedulingNotes`. A checkpoint stage runs no agent: it applies the wave's verified patches to the shared checkout, runs the writers' validation commands, records the cumulative run-owned patch hash, and creates a checkpoint commit on `refs/prompt-chain/...` whose parent is the run base and whose trailer names the wave base. The branch never moves until the final `integrate` stage. Later waves' worktrees branch from that checkpoint commit and their boundary checkpoints are computed against it. Because checkpoint commits use a temporary index, later checkpoints and the final integrate verify the run-owned tree against the previous checkpoint's hash instead of demanding a clean checkout. A checkpoint whose validation fails reverts its patches and pauses the run as `checkpoint_blocked`; a resume re-applies the wave once.
